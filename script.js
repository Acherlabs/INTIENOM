// ============================================
// INTIENOM Website — Main JavaScript (v3.0)
// Naya: FAQ Accordion + data-file-id support
// ============================================
...
// Sirf un pages par chalta hai jahan nav hash-links use karta hai (jaise index.html).
// Multi-page links (contact.html, armor.html) wale pages par HTML mein
// manually class="active" set hota hai — JS use override nahi karega.
function updateActiveNavOnScroll() {
    const navBtns = document.querySelectorAll('.nav-btn');
    if (!navBtns.length) return;

    const isAnchorNav = Array.from(navBtns).some(btn => {
        const href = btn.getAttribute('href') || '';
        return href.startsWith('#');
    });
    if (!isAnchorNav) return;
...
function createParticles() {
    ...
    const colors = ['#FF8A87', '#A31D1C', '#E0605E', '#ffffff'];
        ...
        form.addEventListener("submit", e => {
            ...
            fetch(scriptURL, { method: "POST", body: new FormData(form) })
                .then(response => {
                    ...
                    responseMessage.style.color = "#770A09";
                    form.reset();
                })
                .catch(error => {
...
function initDownloadModal() {
    ...
    // --- NAYA: File ID ab button ke data-file-id attribute se aata hai ---
    // Isse ARMOR aur PipingLab dono apni-apni Drive file download kara sakte hain.
    ...
    const defaultFileId = "1kqtQLNplcL7OV_js9y0YZib0g04ABM1_"; // PipingLab Softwer file Download
    const fileId = openBtn.getAttribute('data-file-id') || defaultFileId;
...
function initScrollReveal() {
    const selectors = [
        ...
        '.product-card', '.metric-card', '.faq-item'
    ];
...
// 10. NAYA: FAQ Accordion (single-open behavior + resize fix)
function initFaqAccordion() {
    ...
    faqItems.forEach(item => {
        ...
        btn.addEventListener('click', function() {
            ...
            // Single-open: baaki sab items band karo
    ...
    // Resize par open answer ki height refresh karo
...
// 10. FAQ Accordion (Single-Open, All Pages)
function initFaq() {
    const faqItems = document.querySelectorAll('.faq-item');
    if (!faqItems.length) return;
    ...
    faqItems.forEach(item => {
        ...
        btn.addEventListener('click', function() {
            ...
            // Single-open: pehle sab band karo
            faqItems.forEach(other => {
                other.classList.remove('active');
                const otherAnswer = other.querySelector('.faq-answer');
                if (otherAnswer) otherAnswer.style.maxHeight = null;
            ...
            // Clicked wala kholo (agar closed tha)
            if (!isOpen) {
                item.classList.add('active');
                answer.style.maxHeight = answer.scrollHeight + 'px';
